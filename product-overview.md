...
Merchants request USDC on Base or Solana. Customers scan a QR code, open a payment link, or pay from a Solana Action / Blink. On Solana, customers can pay with SOL or any supported SPL token — Jupiter routes the swap so the merchant always receives USDC.
...
Invoices become shareable payment links. On Solana, invoice links also expose Action endpoints so they unfurl as payable cards in clients that support Blinks. Each invoice carries a 30-minute expiry, unique reference, and full settlement data for the target chain.
...
| Segment | Use Case |
|---------|----------|
| Freelancers and agencies | Generate payment links, share with clients, receive USDC settlement |
...
**Composable.** Routing, balances, settlement detection, name resolution, and Blinks all use ecosystem-standard providers (Jupiter, Zerion, Helius, SNS, Solana Pay). Secant composes existing infrastructure rather than rebuilding it.
